...
map_grtm <- function(resp, ipar, tlid, cov_p, SE=FALSE, DeBug=FALSE,
                     D=1, bscut=NULL, maxitr=200, 
                     tol=list(nr=c(1.0e-4, 1.0e-3), loglike=1.0e-6),
                     trunc=list(th=c(-3.5, 3.5), gam=c(-3.0, 3.0))){
  ## Estimate MAP under polytomous random-effect testlet model  
  ...
  gam_init <- matrix(NA, nexaminee, ntl)
  for (s in 1:ntl){# s<-1
    ...
    gam_init[tlsc==sum(K_j[iset]),s] <- trunc$gam[2] # is.infinite
  ...
  for (i in 1:nexaminee){# i<-1
    ...
    while (itr < maxitr){
      ...
      dPost  <- matrix(NA, 1, (ntl+1))
      Hessian <- matrix(0, (ntl+1), (ntl+1))
      
      dPost[1] <- sum(D * disc[,1] * u_jki[,,i] * (kmat - lam_j1)) - inv_cov[1,1] * (th_try - mu_p[1])
      Hessian[1,1] <- -sum(D^2 * disc[,1]^2 * u_jki[,,i] *(lam_j2 - lam_j1^2)) - inv_cov[1,1]
      for (s in 1:ntl){ # s<-1
        iset <- which(tlid==tlidu[s])
        dPost[s+1] <- sum(D * disc[iset, 2] * u_jki[iset,,i] * (kmat[iset,] - lam_j1[iset])) - 
          inv_cov[s+1,s+1] * (gam_try[s] - mu_p[s+1])
        ...
        Hessian[1, s+1] <- Hessian[s+1, 1] <- - sum(D^2 * disc[iset, 1] * disc[iset, 2] * u_jki[iset,,i] * 
                                                      (lam_j2[iset] - lam_j1[iset]^2)) - inv_cov[1,s+1]
        ## Note. Cross second-order derivatives bw gammas equal 0.
      }
      
      delta <- dPost %*% MASS::ginv(Hessian)
  ...
  if (isTRUE(SE)){
    ...
    for (i in 1:nexaminee){# i <- 1
      ...
      for (s in 1:ntl){ # s<-1
        ...
        infmat[1, s+1] <- infmat[s+1, 1] <- sum(D^2 * disc[iset, 1] * disc[iset, 2] * 
