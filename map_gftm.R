...
map_gftm <- function(resp, ipar, model="GFTM", var_p=NULL, 
                     SE=FALSE, DeBug=FALSE,
  ...
  for (i in 1:nexaminee){# i<-1
    ...
    while (itr < maxitr){
      ...
      dPost <- sum(D * ipar[,"a"] * u_jki[,,i] * (kmat - lam_j1)) - (1/var_p) * (th_try - mu_p)
      Hessian <- - sum(D^2 * ipar[,"a"]^2 * u_jki[,,i] * (lam_j2 - lam_j1^2)) - (1/var_p)
    ...
    track_itr$nitr[i] <- itr # View(track_itr$nitr)
